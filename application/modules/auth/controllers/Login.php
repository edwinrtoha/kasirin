	...
	public function index() {
		...
		if($login==0){
			...
			if(isset($_GET['act'])){
				...
				$data=array('title'=>$this->main_model->title("LOGIN"),'content'  =>'auth/login_index_view');
					...
					if($query->num_rows()==1){
						foreach($query->result() as $row){
							if($row->secret_key!=""){
								...
								redirect(base_url("auth/login/checkpoint"));
							}
							else{
								$this->Auth_model->login($username,$password, base_url('dashboard'), base_url('auth/login'));
							}
						}
					}
					else{
				...
				$this->load->view("auth/layout/wrapper",$data);
			}
		}
		else{
		...
		if($method=="connect"){
			...
			if($login==0){
				...
				if (!$this->input->get('code')){
					...
					elseif($num_rows==1){
						$this->Auth_model->login_oauth($oauth_uid, base_url('dashboard'), base_url('auth/login'));
					}
				}
			}
			else{
